...
#INDICES = { 'Cb': '11', 'C': 0, 'C#': 1, 'Db': 1, 'D': 2, 'D#': 3, 'Eb': 3, 'E': 4, 'F': 5, 'F#': 6, 'Gb': 6, 'G': 7, 'G#': 8, 'Ab': 8, 'A': 9, 'A#':10, 'Bb':10, 'B':11}
...
class N2C(object):
    ...
    def printDict1(self, d, reason=''):
        print('{}{{'.format(reason), end='')
        for k, v in d.items():
            print('{}:{}'.format(k, v), end=' ')
        print('}', end=' ')
    
    def printDict2(self, d, reason=''):
        print('{}{{'.format(reason), end='')
        for k in d.keys():
            print('{}'.format(k), end=' ')
        print(':', end=' ')
    ...
    def run(self):
        ...
        while True:
            ...
            if c in INDICES:
                index = INDICES[c]
                indices.append(index)
                print('index={} indices={}'.format(index, indices))
                s += c
            elif c == 'b' or c == '#':
                c = s[-1] + c
                index = INDICES[c]
                indices[-1] = index
                print('index={} indices={} c={}'.format(index, indices, c))
                s[-1] = c
            else: raise Exception('Invalid char c = {}:'.format(c))
        print('s={}'.format(s))
        for i in range(len(indices)):
            intervals = self.chordsObj.getIntervals(i, indices)
            imap = {}
            for j in range(len(indices)):
                imap[intervals[j]] = s[j]
            print('i={} intervals={} imap={}'.format(i, intervals, imap))
            chordName = self.chordsObj.getChordName(imap)
            print('chordName={}'.format(chordName))
        print('END run')
'''
